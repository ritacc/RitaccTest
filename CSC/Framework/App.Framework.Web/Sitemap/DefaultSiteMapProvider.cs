...
using App.Framework.Web.MvcSiteMapProvider.External;
...
namespace App.Framework.Web.MvcSiteMapProvider
{
        ...
        : StaticSiteMapProvider
    {
        ...
        private readonly XNamespace ns = "http://mvcsitemap.codeplex.com/schemas/MvcSiteMap-File-3.0";
        private readonly object synclock = new object();
        private string cacheKey = "__MVCSITEMAP_A33EF2B1-F0A4-4507-B011-94669840F79C";
        private string aclCacheItemKey = "__MVCSITEMAP_7C881C5D-9338-4CEF-AF5F-4BA5B31EB1C0";
        private string currentNodeCacheKey = "__MVCSITEMAP_C7704085-E4F5-42C2-BE74-3DD32D360623";
        private SiteMapNode root;
        private bool isBuildingSiteMap = false;
        private bool scanAssembliesForSiteMapNodes;
        private string siteMapFile = string.Empty;
        private string siteMapFileAbsolute = string.Empty;
        private List<string> excludeAssembliesForScan = new List<string>();
        private List<string> includeAssembliesForScan = new List<string>();
        ...
        private void OnSiteMapChanged(string key, object item, CacheItemRemovedReason reason)
        {
            lock (synclock)
            {
                if (key == cacheKey && item != null) // && ((DateTime)item).AddMinutes(CacheDuration).CompareTo(DateTime.Now) <= 0)
                {
                    // Clear the sitemap
                    Clear();
        ...
        {
            if (node.Url.Contains("http") || node.Url.Contains("ftp"))
            {
