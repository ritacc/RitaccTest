CREATE PROCEDURE [dbo].[spSearchRoleInUser]
(
	 @SYS_CODE NVARCHAR(4)
	,@USER_ID INT = 0
    ...
    IF (@USER_TYPE = N'SA') 
        BEGIN	 
            SELECT
               ...
               ,dbo.fnConvertNvarcharToBit(SY_USER_ROLE.ACTIVE_FLAG) AS ACTIVE_FLAG
               ,SY_ROLE.ROLE_ID
            FROM
                SY_ROLE
                LEFT OUTER JOIN SY_USER_ROLE ON SY_USER_ROLE.ROLE_ID = SY_ROLE.ROLE_ID
                                                AND SY_USER_ROLE.[USER_ID] = @USER_ID
                                                AND SY_USER_ROLE.ACTIVE_FLAG = N'Y'
    ...
    ELSE--如果当前用户是公司用户，选择系统层面角色+当前店对应的角色
        BEGIN
            ...
            WHERE
                ...
                AND SY_ROLE.FROZEN_FLAG = N'N'
                AND (SY_ROLE.ROLE_TYPE = N'SY'
                     OR SY_ROLE.SHOP_CODE = @SHOP_CODE
                    )
                --AND SY_ROLE.SHOP_CODE = @SHOP_CODE
            ORDER BY
                SY_USER_ROLE.ACTIVE_FLAG DESC
               ,SY_ROLE.ROLE_TYPE DESC
               ,SY_ROLE.ROLE_CODE ASC
