	...
	$.fn.isHappy = function (config) {
        ...
        errorEl = $(error.id).length > 0 ? $(error.id) : getError(error);
          ...
          //required = !!el.get(0).attributes.getNamedItem('required') || opts.required,
            required = !!el.get(0).attributes.getNamedItem('required') || opts.required || !!el.get(0).attributes.getNamedItem('requireds'),//requireds is my defined,i don't want to use required
          ...
          arg = isFunction(opts.arg) ? opts.arg() : opts.arg;
				...
				if (error) {
					el.addClass('unhappy').after(errorEl); // el.addClass('unhappy').before(errorEl);
					return false;
				} else {
					temp = errorEl.get(0);
					// this is for zepto
