(function (c, f, d, b, e) {
    ...
    var a; d.extend(f, { pageId4Ad: 1, loadIndexAD: function () {
        ...
        , m = j.find(".ctrol")
        , g = "[{img:'" + f.cdnUrl + "/img/index/headFigureDefault.jpg',title:'用网易红包，购彩更省钱',url:'http://caipiao.163.com/sale/coupon_saleCouponIn.html'}]"; 
        var l = function (s) {
            n.addClass("hide"); i.addClass("hide"); k.removeClass("hide");
            ...
            k[0].innerHTML = q.join(""); q = [];
            for (; p < u; p++) {
                t = r[p]; q[p] = "<a " + (p == 0 ? "class='active'" : "") + " href='" + t.url + "' title='" + t.title + "' ii='" + (p + 1) + "' target='_blank'>" + (p + 1) + "</a>"
                  } m[r.length < 2 ? "hide" : "show"]().html(q.join("")); r.length > 1 && f.loadCdnJS("js2/index/scrollImg.js", function () {
                      return !!d.scrollImg
                  }, function () { d.scrollImg({ panlWrap: k, ctrolWrap: m }) })
              };
              this.isStatic ? l(g) : this.shareGet("indexAD", 60 * 1000, h, function (o, p) { if (o) { p = g } l(p) }); j.delegate(".ctrol a,.adList a", "click", function () {
                  var q = this.href, r = this.title, p = d(this).attr("ii"), o = "http://caipiao.163.com/help/special/link" + p + "/"; if (!/\/help\/special\/link/i.test(q)) { f.virualViewStat(o, r) } 
              })
          }, quickInit: function () {
              ...
              this.saveFromInfo(); this.isStatic || c.setTimeout(function () {
                  var g = "http://widget.weibo.com/relationship/followbutton.php?language=zh_cn&width=63&height=22&uid=1792907627&style=1&btn=light&dpc=1";
                  d("#weiboFrame").attr("src", g)
              }, 2000); if (d.getHashPara("addFav") == "1") { d.addFav("http://caipiao.163.com", "网易彩票-国家正规发行 福利彩票 体育彩票") }
              ...
              this.initAwardBox(); d(".latestWin_con").scrollGrid({ interval: 3000 });
              this.adLoader1(d("#advertCenter"), "http://caipiao.163.com/headFigure_getAdImgs.html?memType=pcToutuMiddle", "newad750100");
              this.adLoader(d("#advertShuzi"), "http://caipiao.163.com/help/special/shouyessq/", "shouyessq");
              this.adLoader(d("#advertJingji"), "http://caipiao.163.com/help/special/shouyejingcai/", "shouyejingcai");
              this.adLoader(d("#advertGaopin"), "http://caipiao.163.com/help/special/shouyegaopin/", "shouyegaopin");
              this.adLoader(d("#mobileBox .mobile_popu"), "http://caipiao.163.com/help/special/shouyeshouji/", "shouyeshouji", { url: "http://caipiao.163.com/mobile/client_cp.jsp#fromsy", img: "http://img1.cache.netease.com/help/2013/5/21/20130521144320bbe45.jpg" });
              c.setTimeout(this.accruingAmountsAnimate, 500); this.initGroupbuy(); c.console && c.console.info && this.loadCdnJS("js2/popularize/recruit.js")
          }
          , uaFn: function () {
              a = (function () {
          ...
          , iosFun: function (g) {
              d(g).click(function (i) {
                  var h = +new Date; try { c.location.href = "ntescaipiao://" } catch (i) { } c.setTimeout(function () {
                      if (+new Date - h < 2000) {
                          c.location.href = "http://itunes.apple.com/cn/app/id411654863?mt=8#"
                      } 
...
, adLoader: function (i, g, h, j) {
    f.shareGet(h, 3 * 60 * 1000, g, function (n, m) {
        if (n && j) {
            var k = '<a href="{url}" target="_blank"><img src="{img}" alt="" title="{title}" /></a>'; var l = d.format(k, d.extend({ title: "", img: "", url: "" }, j)); i.html(l); return
        } else { if (n) { i.hide(); return } } m && i.html(d("<div>" + m + "</div>").find("a"))
    })
}
...
var i = d('<span class="providerTip"></span>').appendTo(document.body).hide(); d("#groupBuy").delegate(".order_icon,.provider_link", "mouseenter", function () {
    ...
    var r = d.format, s = d("#highFrequenceAward"), x = d("#tplHighFrequenceAward").html(), j = "http://caipiao.163.com/getAwardWarn.html", n = []; f.loadCdnJS("js2/timer.js", function () { if (d("#highFrequenceAward").length) { q() } }); function m(y, z) { return new v(y, z) } function v(y, z) { this._init(y, z) } v.prototype = { _init: function (y, z) {
...
function h() { s.html(" "); s.unbind() } function g(C, y, A) {
    C.html(" ");
    var B = { jxd11: "http://caipiao.163.com/order/jx11xuan5/", d11: "http://caipiao.163.com/order/11x5/", gdd11: "http://caipiao.163.com/order/gd11x5/" };
